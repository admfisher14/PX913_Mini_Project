module velocity_verlet
        ...
        contains
                ...
                function  get_acc(exact_pos,dx,dy,Ex,Ey) result (acc)
                        ...
                        current_cell(1) = int(floor((exact_pos(1) +1)/dx) + 1) !>I know the pdf says "-1.0_dp"
                        current_cell(2) = int(floor((exact_pos(2) +1)/dy) + 1) !>But the maths seems weird to me here. Correct

                        acc(1) = -1*Ex(current_cell(1),current_cell(2))
                        acc(2) = -1*Ey(current_cell(1),current_cell(2))
